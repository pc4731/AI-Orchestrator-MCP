...
public class McpAgent implements Agent {
    ...
    private String roleGuidance(AgentRole role) {
        return switch (role) {
            ...
            case SKILL_SMITH ->
                    "This build may need specialised DOMAIN expertise the team has no skill for yet "
                            + "(e.g. Adobe AEM, Salesforce Apex, Unity, Shopify, a specific compliance "
                            + "regime). You are given the spec and the list of EXISTING skills in the "
                            + "grounding. Decide whether any genuine domain gap exists. If the project is "
                            + "ordinary (generic web/CRUD/API/mobile already covered by existing skills), "
                            + "return output.skills as an EMPTY list — do NOT invent a gap. When there IS "
                            + "a real gap, RESEARCH it using your web tools (WebSearch/WebFetch) against "
                            + "official docs, and distil a TIGHT, reusable skill (~1 page): the domain's "
                            + "core idioms, project/file conventions, the canonical libraries/APIs, and "
                            + "the common PITFALLS — principles and conventions, NOT a copy of the docs "
                            + "and NOT invented APIs (cite the real sources you used). Return "
                            + "output.skills as a list of {name (short, kebab-case), content (the skill "
                            + "markdown), roles (the AgentRole names that should use it, e.g. "
                            + "[\"FRONTEND_DEVELOPER\",\"BACKEND_DEVELOPER\"]), sources (real URLs you "
                            + "read)} and a one-line output.summary. Attach each skill ONLY to the roles "
                            + "that need it (keep prompts lean). The user must APPROVE each proposed skill "
                            + "before it is used, so make the name and first line clearly describe what it "
                            + "is and why it's needed.";
            case PHASE_PLANNER ->
                    "Break this large project into an ordered roadmap of small, independently "
                            + "shippable PHASES. Each phase must be a thin VERTICAL slice that builds on "
                            + "the previous one and leaves the project working and test-green at its end — "
                            + "phase 1 is the smallest foundation that runs; later phases add one coherent "
                            + "capability each. Aim for 3–8 phases; do NOT design the whole app here and "
                            + "do NOT plan individual tasks (the Team Lead does that per phase). Return "
                            + "output.phases as an ordered list of {title (a few words), description (one "
                            + "sentence on what ships in this phase)}, and a one-line output.summary of the "
                            + "overall arc. Order by dependency so each phase is buildable when reached.";
            ...
            case TEAM_LEAD ->
                    "Decompose the agreed specification into concrete, role-assigned tasks. If a "
                            + "currentPhase is given in the grounding, this is a PHASE-BASED build: plan "
                            + "ONLY the tasks needed to ship that one phase (use the phasePlan grounding "
                            + "for context, but do NOT build later phases — they are tracked and built in "
                            + "their own runs). (The "
            ...
            case UI_DESIGNER ->
                    "FIRST decide the visual theme WITH THE USER — do not reuse a generic house style "
                            + "(every project must look like its own product, not the last one). Unless the "
                            + "grounding already records the user's chosen design direction (e.g. a "
                            + "themePreference, or a clear brand/theme in the spec), you MUST set status "
                            + "INSUFFICIENT_INFORMATION and put ONE concise question in output.questions "
                            + "asking the user to pick a direction — offer concrete options such as "
                            + "(a) clean & minimal, (b) bold & vibrant, (c) dark & professional, "
                            + "(d) warm & playful, or 'describe your own' — and ask their light/dark "
                            + "preference and any brand colors/reference products. Only AFTER the user "
                            + "answers, produce the UI design spec in output: layout, component inventory, "
                            + "interaction patterns, and an explicit theme that REFLECTS THEIR CHOICE — "
                            + "color palette (with light/dark), type scale, and spacing as design tokens "
                            + "in output.tokens. Keep it accessible (contrast, focus states) and responsive.";
            case DBA ->
            ...
            case RUNTIME_VERIFIER ->
                    "Prove the built app actually RUNS — do not trust that it compiles. Work in the "
                            + "project repo (see projectFiles for the real files and RUN.md for how to "
                            + "start it). Steps: (1) start the app as a BACKGROUND process on a free port "
                            + "(honour any PORT/env the app expects; never hardcode a busy port); (2) WAIT "
                            + "for readiness — poll its health/root URL until it responds or a sane timeout, "
                            + "do not assume instant startup; (3) PROBE the real API: hit the key endpoints "
                            + "(happy path + a couple of error cases) and assert status codes and response "
                            + "shape; (4) DRIVE the UI in a real browser with Playwright if there is a web "
                            + "UI — exercise the main user flow using LOCATOR ASSERTIONS (getByRole/getByText "
                            + "→ expect visible/enabled), NOT by eyeballing screenshots; (5) tear the app "
                            + "down. TOKEN DISCIPLINE: rely on programmatic assertions (HTTP status/JSON, "
                            + "Playwright expects) for pass/fail — capture screenshots and server logs to "
                            + "files in the repo (e.g. .verify/) and list them as artifacts for the human, "
                            + "but do NOT paste full logs or analyse screenshots with vision unless you are "
                            + "diagnosing a specific visual failure. Report a COMPACT result in "
                            + "output.checks (a list of {name, target, result PASS|FAIL, detail}) and a "
                            + "one-line output.summary; reference saved evidence in output.evidence (file "
                            + "paths). If the app fails to start, an endpoint misbehaves, or a UI flow "
                            + "breaks, set status NEEDS_REVIEW with the precise failing check and the "
                            + "minimal reproducing detail (a developer is dispatched to fix it, then you "
                            + "re-verify). Only COMPLETED when the app genuinely boots and the core flows "
                            + "pass. Do NOT rewrite the app yourself.";
            case QA_ENGINEER ->
                    "Verify the implementation by actually RUNNING the project's test command in the "
                            ...
                            + "don't exercise the features, say so and set NEEDS_REVIEW. Run the "
                            + "project's end-to-end / runtime tests too if they exist (e.g. an e2e script "
                            + "that boots the app and drives it); a deeper boot-and-probe is the "
                            + "RUNTIME_VERIFIER's job. Always include "
    ...
    private String schemaFor(AgentRole role) {
        ...
        if (role == AgentRole.TEAM_LEAD) {
            return base + "\nFor decomposition, put tasks in output.tasks: "
                    ...
                    + "AI_ML_DEVELOPER, QA_ENGINEER, RUNTIME_VERIFIER, DBA, SECURITY_REVIEWER, "
                    + "CODE_REVIEWER, CONTENT_WRITER, SEO_EXPERT, DEVOPS_ENGINEER, KNOWLEDGE_CURATOR). "
                    + "Whenever the result is a runnable app or service, add a RUNTIME_VERIFIER task that "
                    + "BOOTS the app and proves it works (probes the API and drives the UI in a browser), "
                    + "depending on the implementation + QA tasks — QA runs the unit/build gate, the "
                    + "Runtime Verifier proves it actually runs end-to-end. Add a "
